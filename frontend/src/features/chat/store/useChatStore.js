...
const messageCachePrefix = "realtime-chat-messages:";

const readCachedMessages = (cacheKey) => {
  try {
    return JSON.parse(localStorage.getItem(`${messageCachePrefix}${cacheKey}`)) || [];
  } catch {
    return [];
  }
};

const writeCachedMessages = (cacheKey, messages) => {
  try {
    localStorage.setItem(
      `${messageCachePrefix}${cacheKey}`,
      JSON.stringify(messages.slice(-200))
    );
  } catch {
    // A full or disabled localStorage should not break chat.
  }
};
...
const getSelectionCacheKey = (selection) => selection.type === "group"
  ? `group:${selection._id}`
  : `direct:${getDirectUserId(selection)}`;
...
export const useChatStore = create((set, get) => ({
  ...
  getMessagesBySelection: async (selection) => {
    ...
    set({
      ...
      messageCursor: null,
      hasMoreMessages: Boolean(cachedMessages.length),
    ...
    try {
      ...
      const result = Array.isArray(res.data)
        ...
        : res.data;
      const messages = mergeMessages(cachedMessages, result.messages || []);
  ...
  loadOlderMessages: async () => {
    const { selectedUser, messageCursor, hasMoreMessages, isLoadingOlderMessages, messageCacheKey } = get();
    ...
    try {
      ...
      writeCachedMessages(messageCacheKey, messages);
      set({
        messages,
        messageCursor: result.nextCursor,
        hasMoreMessages: result.hasMore,
  ...
  sendMessage: async (payload) => {
    ...
    try {
      ...
      writeCachedMessages(get().messageCacheKey, currentMessages);
    } catch (error) {
  ...
  subscribeToMessage: () => {
    ...
    socket.on("newMessage", (newMessage) => {
      ...
      set({ messages });
      writeCachedMessages(get().messageCacheKey, messages);
      if (isSoundEnable) {
